$(function () {
    ...
    $.get(basePath + "/assets/customizer.txt", function (html_string) {
...
function loadConfiguration() {
    ...
    const selectedSidebarOption = getLocalStorageItem("sidebar-option", "horizontal-sidebar");
    // El layout Mavoo SIEMPRE usa horizontal-sidebar; respetamos la elección
    // del usuario solo si NO es 'vertical-sidebar' (legacy), sino forzamos horizontal.
    var applied = selectedSidebarOption;
    if (!applied || applied.indexOf('vertical') === 0) {
        applied = 'horizontal-sidebar';
        $("nav").removeClass('vertical-sidebar horizontal-sidebar').addClass('horizontal-sidebar');
    } else {
        $("nav").removeClass('vertical-sidebar horizontal-sidebar').addClass(applied);
    }
    $(".offcanvas-body > ul").find('.' + applied.replace(/\s+/g, '.')).addClass("selected");
