...
// >>-- 06 Sidebar scroll js --<<
// NOTA: SimpleBar crea un wrapper con overflow:hidden que RECORTA los
// submenús desplegables. En un menú horizontal no hay scroll real necesario,
// así que desactivamos SimpleBar sobre #app-simple-bar.
const myElement = document.getElementById('app-simple-bar');
if (myElement) {
    // Comentado para evitar clipping de submenús Bootstrap collapse:
...
// >>-- 14 CloseCollapse js --<<
// Intercepta los EVENTOS de Bootstrap para coordinar submenús.
// Cuando un submenú se abre (shown.bs.collapse), cierra los otros
// forzando display:none para evitar problemas de timing con animaciones.
(function () {
    document.querySelectorAll('.main-nav ul.collapse').forEach(function (submenu) {
        submenu.addEventListener('shown.bs.collapse', function () {
            document.querySelectorAll('.main-nav ul.collapse').forEach(function (other) {
                ...
                // Limpiar transiciones forzadas que Bootstrap aplica
                other.style.transition = 'none !important';
                other.style.height = '';
                other.style.setProperty('display', 'none', 'important');
                other.offsetHeight; // forzar reflow
                other.style.transition = '';
                var trigger = other.previousElementSibling;
                if (trigger) trigger.setAttribute('aria-expanded', 'false');
            });
        });
    });

    // Cerrar todos los submenús al hacer click fuera del nav
    document.addEventListener('click', function (e) {
        if (!e.target.closest('.horizontal-sidebar')) {
            document.querySelectorAll('.main-nav ul.collapse.show').forEach(function (el) {
